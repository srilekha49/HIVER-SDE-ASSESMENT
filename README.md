...
The golden set contains 200 customer messages stratified across support categories from the source data. Each example is hand-labeled with one of the target support intents.
...
To create the initial sampled evaluation set:

Bash
python create_golden_set.py
This generates data/golden_set.csv without modifying the raw dataset.

To launch the CLI annotation tool:
...
Outputs performance metrics, classification reports, and saves predictions to evaluation_results.csv.

Current Final Results
Metric	Result
Accuracy	86.50%
Macro Precision	0.8510
Macro Recall	0.8380
Macro F1	0.8420
...
Uses deterministic keyword lookup rules across categories. Results are saved to trivial_baseline_results.csv.
...
Measures vector search performance and saves output to retrieval_evaluation_results.csv.
...
Proportion with Similarity ≥0.30: 94.5%

Proportion with Similarity ≥0.50: 78.0%
...
Generates failure_analysis.csv to highlight prediction errors, low similarity matches, and hypothesis tags.
...
Bash
python llm_judge.py
Evaluates replies along four dimensions: Relevance, Groundedness, Helpfulness, and Overall Quality. Saves output to reply_quality_evaluation.csv.
...
22. Project Files
Plaintext
Hiver-sde-assessment/
├── data/
│   ├── spotify_support_pairs.csv
│   └── golden_set.csv
├── support_agent.py
...
23. Reproducibility
Random seeds across data sampling and train-test splits are locked:

Python
random_state = 42
This guarantees identical sampling of the 200 golden-set candidates across all test environments.
